...
function useCalendar(reminders, onDayClick) {
  ...
  const getBlanksFields = () => {
    ...
    for (let index = 0; index < firstDayOfMonth(); index++) {
      blanks.push(
        <td className="calendar-day empty" key={`${index}-empty`}>
          {""}
        </td>
      );
    }
    ...
    for (let day = 1; day <= monthDays(); day++) {
      ...
      daysInMonth.push(
        <td
          key={day}
          id={`calendar-day-${day}`}
          className={`calendar-day ${current} ${beforeToday}`}
        >
          <span className="day-span">{day}</span>
          {remindersArray
            .filter((item) => item.day === day.toString())
            .map((item, key) => {
              ...
              return (
                <div
                  ...
                  className={`reminder-container  ${item.color}-bg`}
                  onClick={(e) => onDayClick(e, item.id)}
                >
                  <div
                  ...
                  >
                    <p
                      className={`reminder-title cut-text-overflow ${isBgYellow}`}
                    >
                      {item.reminder}
                    </p>
                    {item.weather && (
